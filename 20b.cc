...
int main() {
  ...
  auto& rx_pred = system.modules[predecessor];
  std::transform(rx_pred.conjunction_state.begin(), rx_pred.conjunction_state.end(), std::back_inserter(predecessors), [](auto& pair) { return pair.first; });

  std::cout << "Predecessors:";
  for (auto& name : predecessors) {
    std::cout << ' ' << name;
  }
  std::cout << '\n';
  ...
  for (;;) {
    ...
    while (!q.empty()) {
      ...
      if (std::find(predecessors.begin(), predecessors.end(), name) != predecessors.end()) {
        if (pulse == Pulse::Low) {
          std::cout << push << ": " << name << '\n';

          if (cycles.find(name) == cycles.end()) {
            cycles.emplace(name, push);

            if (cycles.size() == predecessors.size()) {
              break;
            }
          }
