...
int main() {
  ...
  for (std::size_t j = 1; j < bricks.size(); ++j) {
    auto& brick = bricks[j];
    ...
    std::set<std::size_t> would_fall;
    would_fall.insert(j);

    std::queue<std::size_t> queue;

    for (auto& above : brick.above) {
      queue.push(above);
    }
    ...
    while (!queue.empty()) {
      ...
      std::set_difference(bricks[i].beyond.begin(), bricks[i].beyond.end(), would_fall.begin(), would_fall.end(), std::back_inserter(difference));

      if (difference.empty()) {
        would_fall.insert(i);

        for (auto& above : bricks[i].above) {
          queue.push(above);
        }
      }
    }

    std::cout << "Brick " << j << ": " << would_fall.size() - 1 << " would fall.\n";
    count += would_fall.size() - 1;
  }
