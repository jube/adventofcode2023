...
namespace {
  ...
  long compute_differences(const std::vector<long>& values) {
    ...
    long last = compute_differences(differences);
    std::cout << "Last: " << last << '\n';
    return last + differences.back();
  }
...
int main() {
  ...
  for (std::string line; std::getline(std::cin, line); ) {
    ...
    long last = compute_differences(initial_values);
    std::cout << "Last: " << last << '\n';
    long prediction = last + initial_values.back();
